...
export default class Arrow extends Phaser.GameObjects.Graphics {
  ...
  beginPoint2!: Phaser.Math.Vector2;
  line: Phaser.Geom.Line;
  line2: Phaser.Geom.Line;
  constructor(scene: Phaser.Scene) {
    super(scene);
    this.beginPoint = new Phaser.Math.Vector2(
      CONST.shootingPointx - 25,
      CONST.shootingPointy
    );
    this.beginPoint2 = new Phaser.Math.Vector2(
      CONST.shootingPointx + 25,
      CONST.shootingPointy
    );
  ...
  init() {
    ...
    this.scene.input.on("pointermove", (pointer) => {
      if (pointer.isDown && pointer.y < CONST.shootingPointy - 20) {
        this.clear();
        this.lineStyle(2, 0xaa00aa);
        this.line.x2 = pointer.x;
        this.line.y2 = pointer.y;
        const leftLineBound = CONST.worldLeftBound +25;
        const leftBounceBound = CONST.worldLeftBound;
        const rightBounceBound = CONST.worldRightBound;
        const rightLineBound = CONST.worldRightBound -25;
        // if x < 140
        if (pointer.x < leftLineBound) {
          ...
          line2.x1 = leftLineBound;
          line2.y1 =
            this.line.y1 -
            ((this.line.x1 - leftLineBound) * velocityVector.y) / velocityVector.x;
          ...
          const velocityVector2 = new Phaser.Math.Vector2(
            line2.x2 - leftBounceBound,
            line2.y2 - line2.y1
          );
          // conculate yB
          const yB =
            line2.y1 -
            ((line2.x1 - rightBounceBound) * velocityVector2.y) / velocityVector2.x;
          const xB = rightLineBound;
          if (yB > 0) {
            line2.x2 = xB;
            line2.y2 = yB;
          }
        ...
        if (pointer.x > rightLineBound) {
          ...
          const line2 = new Phaser.Geom.Line(
            rightLineBound,
            this.line.y1 -
              ((this.line.x1 - rightLineBound) * velocityVector.y) /
                velocityVector.x
          );
          this.line.x2 = line2.x1;
          this.line.y2 = line2.y1;
